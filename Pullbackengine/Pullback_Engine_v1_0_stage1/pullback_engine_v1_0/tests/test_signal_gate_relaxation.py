...
def make_5m():
    ...
    for i in range(40):
        close = 100.0 + i * 0.5
        rows.append(Candle(start + timedelta(minutes=5 * i), close, close + 0.5, close - 0.5, close, 1000.0))
    return rows
...
def test_trigger_does_not_require_nifty_regime_or_ep_threshold():
    ...
    stock = StockData("TEST", "a", make_1m([100.0, 99.0, 98.0, 110.0]), [], False, True, datetime(2026, 9, 12, 10, 3, tzinfo=IST))
...
def test_cp2_keeps_current_stock_usable_when_one_historical_row_is_invalid():
    ...
    stock = engine._stock("a", "TEST", {"1m": rows}, datetime(2026, 9, 12, 9, 18, tzinfo=IST))
